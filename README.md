...
Зачем обновлять мне мною собранное ядро точнее его хедер до той же версии linux-headers-5.6.0-1020-oem ? Все кто пытался собрать это ядро итак знают что оно более не собирается , а модули которые себе за проделанную работу берет облачный клоуд я уже скинул сюда потому что сохранил у себя локально  https://github.com/Griggorii/linux-image-oem-5.6.0-1020-kernel-ubuntu-x86_64-deb-package/blob/master/nvidia-440.100-zfs-my-build-driver.tar.xz работу компилятора типа не учитываем в разработку , а потом как и в случае с qt4 и кернел уже не собирается. Обход этого псевдо обновления сначала поставить python3-distutils python3-lib2to3 затем отдельная скачка пакетов spl-dkms_0.8.3-1ubuntu12.4_all.deb zfs-dkms_0.8.3-1ubuntu12.4_all.deb и установка от рут или sudo dpkg -i *.deb в папке с этими деб пакетами тем самым будет сгенерированный сорс с моего оригинального ядра.
